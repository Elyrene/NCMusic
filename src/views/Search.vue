...
const fetchSearchPlaylist = async () => {
    loading.value = true;
    const keyword = searchKeyWord.value;
    if(!keyword) return;
    try {
        const data = await api.get<SearchRes>('/search', {keywords: keyword});
        tracks.value = data.result.songs.map((item) => ({
            id: item.id,
            name: item.name,
